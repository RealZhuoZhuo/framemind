...
export function getShotGenerationSystemPrompt() {
  return [
    ...
    "如果一个镜头包含多名角色的对话，assetNames 必须包含所有说话角色对应的 character 资产；否则必须拆成多个镜头或删除无法绑定角色的台词。",
    "严禁出现“assetNames 里没有 A 角色，但 dialogue 是 A 说的话”的情况；输出前必须逐条自检 dialogue 的说话者是否在 assetNames 中。",
    "assetNames 中的 character 资产必须和 dialogue 的说话者一致，不要用其他角色资产代替说话者。",
...
export function getShotGenerationUserPrompt(params: {
  ...
  return [
    ...
    "4. 如果 dialogue 包含多名说话角色，assetNames 必须包含所有说话角色；不能只绑定其中一个角色。",
    "5. 严禁让未绑定在 assetNames 里的角色说话；不确定说话者或找不到对应 character 资产时，dialogue 输出空字符串。",
    "6. characterAction 必须来自下方剧本内容里的角色动作，不能为了分镜效果新编动作。",
    "7. characterAction 中出现的角色也必须包含在 assetNames 的 character 资产里；找不到该角色资产时不要写该角色动作。",
    "8. shotDescription 可以组织镜头语言，但不能改变剧本事实。",
    "",
    "输出格式示例：",
