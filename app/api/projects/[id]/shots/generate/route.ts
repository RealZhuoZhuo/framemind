...
import { characterRepo, projectRepo, shotRepo } from "@/lib/repositories";
...
export async function POST(
  ...
  try {
    ...
    if (characters.length === 0) {
      return badRequest("Generate characters before generating storyboard shots");
    }

    const generatedShots = await generateShotsFromScript(script, characters);
